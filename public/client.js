...
socket.on("cardPlayed", data => {
    showPlayEffect(data.card, data.playerId);
});
...
socket.on("unoCalled", data => {
    if (data.playerId === socket.id) {
        ...
        showMessage(data.playerName + " falou UNO!");
    }
});
...
function updateLobby() {
    ...
    if (state.roomCode) {
        ...
        start.title = `É necessário ter ${state.maxPlayers || 4} jogadores para iniciar.`;
        start.style.display =
            state.hostId === socket.id && state.players.length === (state.maxPlayers || 4)
            ? "block"
            : "none";
    }
}
...
function showPlayEffect(card, playerId) {
    // The state update immediately after this event redraws the board.
    // This small visual pulse makes a played card feel more responsive.
    const pile = document.getElementById("discardPile");
    if (!pile) return;

    pile.animate(
        [
            { transform: "scale(.82) rotate(-3deg)", opacity: .5 },
