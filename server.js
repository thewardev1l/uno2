...
function startTurnTimer(room) {
    clearTurnTimer(room);
    if (!room.started || room.players.length < 2 || room.waitingColor || room.waitingSwap) return;

    room.turnTimer = setTimeout(() => {
        if (!room.started || room.waitingColor || room.waitingSwap) return;
        ...
        room.turnInterval = setInterval(() => {
            if (!room.started || room.waitingColor || room.waitingSwap) {
                clearTurnTimer(room);
                return;
            }
...
function sanitize(room, socketId) {
    ...
    return {
        ...
        winner: room.winner,
        mode: room.mode,
        modeName: room.mode === "zeroSeven" ? "0-7" : "Clássico",
        meIndex,
        ...
        waitingSwap: room.waitingSwap && room.pendingSwapPlayerId === socketId,
        swapTargets: room.waitingSwap && room.pendingSwapPlayerId === socketId
            ? room.players.filter(p => p.id !== socketId).map(p => ({ id: p.id, name: p.name, avatar: p.avatar || null, handCount: p.hand.length }))
            : [],
...
function startGame(room) {
    ...
    room.waitingSwap = false;
    room.pendingSwapPlayerId = null;
...
function rotateHandsForZero(room) {
    const hands = room.players.map(player => player.hand);
    const count = room.players.length;
    const rotated = new Array(count);

    for (let i = 0; i < count; i++) {
        const targetIndex = nextPlayer(room, i);
        rotated[targetIndex] = hands[i];
    }

    room.players.forEach((player, i) => {
        player.hand = rotated[i];
        player.drawnCardIndex = null;
        player.unoDeclared = false;
    });
}

function finishZeroSevenAction(room, card, player) {
    if (room.mode !== "zeroSeven") return null;

    if (card.type === "number" && card.value === "0") {
        rotateHandsForZero(room);
        io.to(room.code).emit("zeroSevenZero", {
            playerId: player.id,
            playerName: player.name,
            direction: room.direction
        });
    ...
    if (card.type === "number" && card.value === "7") {
        room.waitingSwap = true;
        room.pendingSwapPlayerId = player.id;
        return { type: "seven" };
    }
...
function playCard(room, player, index) {
    ...
    if (room.waitingSwap) {
        return { ok: false, error: "Escolha um jogador para trocar suas cartas." };
    }
    ...
    const special = finishZeroSevenAction(room, card, player);
    if (special?.type === "seven") {
        return { ok: true, card, needsSwap: true, unoCelebration: declaredUno && player.hand.length === 1 };
    }

    if (special?.type === "zero") {
        const winnerIndex = room.players.findIndex(p => p.hand.length === 0);
        if (winnerIndex >= 0) {
            room.winner = room.players[winnerIndex].name;
            room.started = false;
            return { ok: true, card, zeroAction: true, winner: room.winner, unoCelebration: false };
        }
    }
...
io.on("connection", socket => {
    socket.on("createRoom", ({ name, maxPlayers, avatar, mode }) => {
        ...
        const gameMode = ["classic", "zeroSeven"].includes(String(mode)) ? String(mode) : "classic";
    ...
    socket.on("joinRoom", ({ name, code, avatar, mode }) => {
        const playerName = String(name || "").trim().slice(0, 18);
        ...
        if (["classic", "zeroSeven"].includes(String(mode)) && room.mode !== String(mode)) {
            return sendError(socket, `Essa sala está no modo ${room.mode === "zeroSeven" ? "0-7" : "Clássico"}.`);
        }
    ...
    socket.on("playCard", ({ index }) => {
        ...
        io.to(room.code).emit("cardPlayed", {
            ...
            needsSwap: !!result.needsSwap,
            zeroAction: !!result.zeroAction
        });
    ...
    socket.on("chooseSwapTarget", targetId => {
        const room = rooms.get(socket.data.roomCode);
        if (!room || !room.started) return;
        if (!room.waitingSwap || room.pendingSwapPlayerId !== socket.id) {
            return sendError(socket, "Você não pode trocar as cartas agora.");
        }

        const player = room.players.find(p => p.id === socket.id);
        const target = room.players.find(p => p.id === String(targetId));
        if (!player || !target || target.id === player.id) {
            return sendError(socket, "Escolha outro jogador.");
        }
        ...
        io.to(room.code).emit("zeroSevenSeven", {
            playerId: player.id,
            playerName: player.name,
            targetId: target.id,
    ...
    socket.on("drawCard", () => {
        ...
        if (room.waitingSwap) {
            return sendError(socket, "Escolha um jogador para trocar suas cartas.");
        }
