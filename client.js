...
async function createRoom() {
    ...
    socket.emit("createRoom", { name, maxPlayers: selectedPlayers, avatar });
}
...
function showDrawChoice() {
    ...
    if (unoButton) unoButton.style.display = state && state.hand.length === 2 ? "block" : "none";
    modal.style.display = "flex";
}
...
function declareUnoAndPlayDrawn() {
    ...
    if (state.hand.length !== 2) return showMessage("A opção UNO só está disponível quando você tinha 1 carta antes da compra.");
    const index = state.drawnCardIndex;
    closeDrawChoice();
